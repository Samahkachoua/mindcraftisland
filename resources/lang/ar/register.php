<?php

return [

    // Page
    'page_title'          => 'التسجيل — جزيرة مايند كرافت',
    'heading_prefix'      => 'انضم إلى',
    'subtitle'            => 'أكمل النموذج أدناه للتسجيل في البرنامج.',

    // Field labels
    'full_name'                  => 'الاسم الثلاثي',
    'phone_number'               => 'رقم الهاتف',
    'emergency_contact_number'   => 'رقم جهة اتصال الطوارئ',
    'mother_name'                => 'اسم الأم',
    'field_of_interests'  => 'الحقول المهمة',
    'date_of_birth'       => 'تاريخ الميلاد',
    'medical_conditions'      => 'المشاكل الصحية (إن وجدت)',

    // Placeholders
    'full_name_ph'                    => 'مثال: أميرة كريم حسن',
    'phone_number_ph'                 => 'مثال: 03147852',
    'emergency_contact_number_ph'     => 'مثال: 03147852',
    'mother_name_ph'                  => 'مثال: فاطمة سعيد ',
    'field_of_interests_ph' => 'مثال: ألعاب الطاولة، القراءة، كرة القدم',
    'medical_conditions_ph'     => 'مثال: الحساسية، أمراض مزمنة',

    // Tags
    'required'            => 'مطلوب',

    // Consent
    'photo_video_consent' => 'أوافق على مشاركة صور ومقاطع فيديو طفلي على وسائل التواصل الاجتماعي.',

    // Button
    'submit'              => 'إرسال التسجيل',

    // Age validation
    'dob_too_young'       => 'يجب أن يكون عمر الطفل أكبر من 8 سنوات.',

    // Flash messages
    'success'             => 'تم إرسال التسجيل بنجاح! سنتواصل معك قريباً.',
    'error_duplicate'     => 'يوجد تسجيل بهذا الاسم ورقم الهاتف مسبقاً.',
    'error_generic'       => 'تعذّر حفظ تسجيلك. يرجى المحاولة مرة أخرى.',

    // Language switcher
    'switch_lang_label'   => 'English',
    'switch_lang_locale'  => 'en',

    // Registration type
    'registration_type_heading' => 'من الذي تقوم بتسجيله؟',
    'registration_type_kid'     => 'تسجيل الطفل (6 – 8 سنة)',
    'registration_type_child'   => 'تسجيل الشباب (8+ – 18 سنة)',
    'registration_type_lady'    => 'تسجيل برنامج السيدات (18 سنة فما فوق)',

    // Lady-flow field label overrides
    'full_name_lady'            => 'الاسم الثلاثي',
    'phone_number_lady'         => 'رقم الهاتف',

    // Consent (lady flow)
    'photo_video_consent_lady'  => 'أوافق على مشاركة صوري ومقاطع الفيديو الخاصة بي على وسائل التواصل الاجتماعي.',

    // Age validation
    'dob_child_too_old'         => 'يجب ألا يتجاوز عمر الطفل 18 سنة.',
    'dob_lady_min'              => 'يجب أن يكون عمر المشاركة 18 سنة على الأقل.',
    'dob_kid_too_young'         => 'يجب أن يكون عمر الطفل 6 سنوات على الأقل.',
    'dob_kid_too_old'           => 'يجب ألا يتجاوز عمر الطفل 8 سنوات.',

];
